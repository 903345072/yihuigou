...
class UserRecharge extends BaseModel
{
    ...
    public static function o2ozf($rechargeInfo,$type){
        ...
        //验证key,不可以更改参数顺序。
        $prices = $rechargeInfo['price'];    //注意：020支付需要的参数单元为分;
        $keys = strtoupper(md5("order_no=" . $orderid . "&subject=" . $goodsname . "&pay_type=" . $type . "&money=" . sprintf("%.2f",$prices) . "&app_id=" . $app_id . "&extra=" . $extra . "&" . $app_secret));
        $returndata['money'] = sprintf("%.2f",$prices);
        $returndata['pay_type'] = $type;
        $returndata['subject'] = $goodsname;
        $returndata['order_no'] = $orderid;
        $returndata['app_id'] = $app_id;
        $returndata["extra"] = $extra;

        $returndata['sign'] = $keys;
        return $returndata;
        return "<form style='display:none;' id='formpay' name='formpay' method='post' action='https://gateway.xddpsay.com'>
		<input name='order_no' id='order_no' type='text' value='$orderid'/>
        <input name='subject' id='subject' type='text' value='$goodsname' />
        <input name='pay_type' id='pay_type' type='text' value='$type' />
        <input name='money' id='money' type='text' value='$prices'/>
        <input name='app_id' id='app_id' type='text' value='$app_id'/>        
        <input name='extra' id='extra' type='text' value='$extra'/>
		<input name='sign' id='sign' type='text' value='$keys'/>
