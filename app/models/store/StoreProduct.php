...
//        if($salesOrder) $baseOrder = $salesOrder == 'desc' ? 'sales DESC' : 'sales ASC';//真实销量
    ...
    public static function getNewProduct($field = '*', $limit = 0, $uid = 0, bool $bool = true)
    {
        ...
        $model = self::where('is_new', 1)->where('is_del', 0)->where('is_intergral_pro', 0)->where('mer_id', 0)
     ...
     * @return false|\PDOStatement|string|\think\Collection
     */
    public static function getIntegralPoduct($field = '*', $limit = 0, $uid = 0)
    {
        $model = self::where('is_del', 0)->where('is_intergral_pro', 1)->where('mer_id', 0)
            ->where('stock', '>', 0)->where('is_show', 1)->field($field)
            ->order('sort DESC, id DESC');
        if ($limit) $model->limit($limit);
        $list = $model->select();
    ...
    {
        if (!$limit && !$bool) return [];
        $model = self::where('is_best', 1)->where('is_del', 0)->where('is_intergral_pro', 0)->where('mer_id', 0)
            ->where('stock', '>', 0)->where('is_show', 1)->field($field)
            ->order('sort DESC, id DESC');
        if ($limit) $model->limit($limit);
        $list = $model->select();
    ...
    public static function getBenefitProduct($field = '*', $limit = 0)
    {
        $model = self::where('is_benefit', 1)
            ->where('is_del', 0)->where('mer_id', 0)->where('is_intergral_pro', 0)->where('stock', '>', 0)
