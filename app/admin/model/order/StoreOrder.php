...
class StoreOrder extends BaseModel
{
    ...
    public static function OrderList($where)
    {
        ...
        $model = self::getOrderWhere($where, self::alias('a')
            ...
            ->field('a.*,r.nickname,r.phone,r.spread_uid,r.pid');
        if ($where['order'] != '') {
        ...
        foreach ($data as &$item) {
            ...
            $item['_info'] = $_info;
            $item["back_money"] = Db::table("eb_user_bill")->where(["link_id"=>$item["id"],"type"=>"brokerage"])->value("number");
            ...
            } else if ($item['status'] == 0 && $item['paid'] == 1) {
                $item['status_name'] = '待操作';
            }  else if ($item['status'] == 1 && $item['pin_status'] == 0) {
                $item['status_name'] = '拼团失败';
            }  else if ($item['status'] == 1 && $item['pin_status'] == 1) {
                $item['status_name'] = '拼团成功';
            }else if ($item['status'] == 2 ) {
                $item['status_name'] = '已完成(兑换积分)';
...
HTML;
    ...
    public static function statusByWhere($status, $model = null, $alert = '')
    {
        ...
        if ('' === $status)
            ...
            return $model->where($alert . 'paid', 1)->where($alert . 'status', 3)->where($alert . 'shipping_type', 1)->where($alert . 'refund_status', 0)->where($alert . 'is_del', 0);
        else if ($status == 4)//待收货
            return $model->where($alert . 'paid', 1)->where($alert . 'status', 4)->where($alert . 'shipping_type', 1)->where($alert . 'refund_status', 0)->where($alert . 'is_del', 0);
        else if ($status == 5)// 交易完成
            return $model->where($alert . 'paid', 1)->where($alert . 'status', '2,5')->where($alert . 'refund_status', 0)->where($alert . 'is_del', 0);
        else
            return $model;
    }
...
//        $model = $model->where('combination_id',0);
        ...
        if ($power >=9999){
            ...
            if (isset($where['operation_id']) && $where['operation_id'] != ''){
                ...
                $model->where($aler . 'uid', "in", $user_ids);
            }
        }
