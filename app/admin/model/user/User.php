    ...
    public static function getUserList($where)
    {
        ...
        $list = $model->alias('u')
            ...
            ->each(function ($item) {
                ...
                if ($item['pid']) {
                    $item['spread_uid_nickname'] = SystemAdmin::where(['id'=>$item['pid']])->value('real_name');
                } else {
                    $item['spread_uid_nickname'] = '无';
                }
        ...
        $count = self::setWherePage(self::setWhere($where), $where, ['u.status', 'u.is_promoter'], ['u.nickname', 'u.uid'])->alias('u')->count();
