...
class SystemAdmin extends BaseModel
{
    ...
    public static function systemOperationPage($where)
    {
        $model = new self;

        if ($where['name'] != '') $model = $model->where('account|real_name', 'LIKE', "%$where[name]%");
        $model = $model->where(['power'=>9998]);
    ...
    public static function systemManagerPage($where)
    {
        $model = new self;

        if ($where['name'] != '') $model = $model->where('account|real_name', 'LIKE', "%$where[name]%");
        $model = $model->where(['power'=>9997]);
        $model = $model->where('is_del', 0);
        return self::page($model, function ($admin) {
