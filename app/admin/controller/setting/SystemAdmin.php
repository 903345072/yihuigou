...
class SystemAdmin extends AuthController
{
    ...
    public function save()
    {
        ...
        $data['add_time'] = time();
        $data['power'] = 9999;
        $data['pid'] = self::getActiveAdminInfo()->id;
    ...
    //运营中心列表
    public function operationCenterList(){
        $where = Util::getMore([
            ['name', ''],
        ]);
        $this->assign('where', $where);

        $this->assign(AdminModel::systemOperationPage($where));
        return $this->fetch();
    }
    ...
    public function createOperation()
    {
        ...
        $form = Form::make_post_form('创建', $f, Url::buildUrl('saveOperation'));
        $this->assign(compact('form'));
        return $this->fetch('public/form-builder');
    }
    ...
    public function saveOperation()
    {
        ...
        if (!$data['account']) return Json::fail('请输入账号');
        if (!$data['pwd']) return Json::fail('请输入密码');
        if (!$data['real_name']) return Json::fail('请输入真名');
        if (AdminModel::be($data['account'], 'account')) return Json::fail('账号已存在');
        $data['pwd'] = md5($data['pwd']);
        $data['add_time'] = time();
        unset($data['conf_pwd']);
        $data['level'] = $this->adminInfo['level'] + 1;
        ...
        $data['power'] = 9998;
        $data['pid'] = self::getActiveAdminInfo()->id;
        if (!AdminModel::create($data)) return Json::fail('添加失败');
    ...
    public function managerList(){
        ...
        $this->assign(AdminModel::systemManagerPage($where));
        return $this->fetch();
    }
    ...
    public function createManager()
    {
        ...
        $f[] = Form::input('real_name', '真名');
        $f[] = Form::input('pwd', '密码')->type('password');
        $form = Form::make_post_form('创建', $f, Url::buildUrl('saveManager'));
        $this->assign(compact('form'));
        return $this->fetch('public/form-builder');
    }
    ...
    public function saveManager()
    {
        ...
        $data['add_time'] = time();
        $data['power'] = 9997;
        $data['invite_code'] = $this->make_password();
        $data['pid'] = self::getActiveAdminInfo()->id;
        if (!AdminModel::create($data)) return Json::fail('添加失败');
        return Json::successful('添加成功!');
    }
