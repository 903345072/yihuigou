...
//        $priceGroup = StoreOrder::getOrderPriceGroup($cartInfo);
        ...
        $priceGroup = StoreOrder::cacheKeyCreateOrder("0",$request->uid(), $key, $addressId, $payType, (int)$useIntegral, $couponId, $mark, $combinationId, $pinkId, $seckill_id, $bargainId, true, 0, $shipping_type);
        if ($priceGroup)
    ...
    public function create(Request $request, $key)
    {
        ...
        if (StoreOrder::be(['order_id|unique' => $key, 'uid' => $uid, 'is_del' => 0]))
            return app('json')->status('extend_order', '订单已生成', ['orderId' => $key, 'key' => $key]);
        list($addressId, $couponId,$lucky_number, $payType, $useIntegral, $mark, $combinationId, $pinkId, $seckill_id, $formId, $bargainId, $from, $shipping_type, $real_name, $phone, $storeId) = UtilService::postMore([
            'addressId', 'couponId',"lucky_number", 'payType', ['useIntegral', 0], 'mark', ['combinationId', 0], ['pinkId', 0], ['seckill_id', 0], ['formId', ''], ['bargainId', ''], ['from', 'weixin'],
        ...
        elseif ($from == 'weixinh5')
            $isChannel = 2;
        $order = StoreOrder::cacheKeyCreateOrder($lucky_number,$request->uid(), $key, $addressId, $payType, (int)$useIntegral, $couponId, $mark, $combinationId, $pinkId, $seckill_id, $bargainId, false, $isChannel, $shipping_type, $real_name, $phone, $storeId);
...
//            event('ShortMssageSend', [$orderId, 'AdminPlaceAnOrder']);//发送管理员通知
    ...
    public function lst(Request $request)
    {
        ...
        $kj_time = Db::table("eb_turntable")->order("id desc")->value("add_time");
        $kj_time = 300-(time()-$kj_time);
        return app('json')->successful(["data"=>StoreOrder::getUserOrderSearchList($request->uid(), $type, $page, $limit, $search),"kj_time"=>$kj_time]);
    }
    ...
    public function takeGoods(Request $request)
    {
        ...
        if (!$uni) return app('json')->fail('参数错误!');
        $res = StoreOrder::takeGoods($uni, $request->uid());
        if ($res) {
    ...
    public function returngoods(Request $request)
    {
        ...
        if (!$uni) return app('json')->fail('参数错误!');
        $res = StoreOrder::returngoods($uni, $request->uid());
        if ($res) {
     ...
     * @return mixed
     */
    public function ChargeIntegral(Request $request)
    {
        list($uni) = UtilService::postMore([
            ['uni', ''],
        ], $request, true);
        if (!$uni) return app('json')->fail('参数错误!');
        $res = StoreOrder::ChargeIntegral($uni, $request->uid());
        if ($res) {
            return app('json')->successful();
        } else
            return app('json')->fail(StoreOrder::getErrorInfo());
