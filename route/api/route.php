...
Route::get('setUserInfo', 'AuthController/setUserInfo')->name('setUserInfo')
    ->middleware(\app\http\middleware\AllowOriginMiddleware::class);   
...
//    Route::post('like/del', 'user.UserController/like_del')->name('likeDel');//取消点赞
    ...
    Route::post('order/takeGoods', 'order.StoreOrderController/takeGoods')->name('takeGoods'); //提货
    Route::post('order/returngoods', 'order.StoreOrderController/returngoods')->name('returngoods'); //退货
    Route::post('order/ChargeIntegral', 'order.StoreOrderController/ChargeIntegral')->name('takeGoods'); //订单兑换对分
