  ...
  <div class="order-submission">
    ...
    <div class="moneyList">
      <div
        ...
        v-if="orderPrice.total_price !== undefined && orderPrice.total_price>0"
      >
        <div>商品总价：</div>
...
.order-submission .allAddress {
  width: 100%;
  background-image: linear-gradient(to bottom, #ff1f44 0%, #f5f5f5 100%);
  background-image: -webkit-linear-gradient(
    to bottom,
    #ff1f44 0%,
    #f5f5f5 100%
  );
  background-image: -moz-linear-gradient(to bottom, #ff1f44 0%, #f5f5f5 100%);
}
...
export default {
  ...
  props: {},
  computed: {
    ...mapGetters(["storeItems", "longitude", "latitude"]),
    storeItem: function() {
  ...
  mounted: function() {
    ...
    if (that.$route.params.id !== undefined)
      ...
      that.lucky_number = that.$route.params.lucky_number;
  },
  methods: {
    // 获取门店列表数据
    getList: function() {
