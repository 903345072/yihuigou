...
//退货
export function returnGoods_(uni) {
  return request.post("/order/returngoods", { uni });
}
//提货
export function takeGoods_(uni){
  return request.post("/order/takeGoods", { uni });
}

//兑换积分
export function takeChargeIntegral_(uni){
  return request.post("/order/ChargeIntegral", { uni });
}
/**
 * 删除订单
 * @returns {*}
